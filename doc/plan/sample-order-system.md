...
| 25 | model+controller+view | 생산라인 실시간 진행 (경과시간 추산 + 조회 화면 전용 스레드 + 상세정보) | [phase-25](sample-order-system/phase-25-production-line-live-tick.md) | [x] |
...
| 35 | controller | 승인/거절/취소 진입시 대기목록 먼저 표시 + 서브메뉴 취소 옵션 | [phase-35](sample-order-system/phase-35-show-pending-before-action.md) | [x] |
| 36 | controller | 주문ID 포맷 변경 (O1 → ORD-001, O/0 혼동 방지) | [phase-36](sample-order-system/phase-36-order-id-format.md) | [x] |
| 37 | controller+view | 시료 조회/검색 결과에 재고수량 포함 | [phase-37](sample-order-system/phase-37-sample-list-with-inventory.md) | [x] |
| 38 | controller | 주문 상태별 카운트에 0건 상태도 전부 표시 | [phase-38](sample-order-system/phase-38-status-count-all-statuses.md) | [x] |
