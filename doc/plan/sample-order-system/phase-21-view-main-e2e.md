...
- [x] 시료관리 메뉴 콘솔 흐름 테스트 (Phase17/22에서 이미 검증됨 — 회귀만 확인, 수동)
- [x] 주문 메뉴 콘솔 흐름 테스트 (접수/승인/거절/취소, 수동)
- [x] 모니터링 메뉴 콘솔 흐름 테스트 (수동)
- [x] 출고처리 메뉴 콘솔 흐름 테스트 (수동, CONFIRMED 아닌 상태에서 안전하게 거부되는지 포함)
- [x] 생산라인 메뉴 콘솔 흐름 테스트 (정적 조회만, 실시간 갱신은 Phase25, 수동)

## 수동 확인 중 발견/처리한 이슈

- 재고가 아예 등록 안 된 시료로 주문 승인 시 `AttributeError`로 콘솔이 죽는 버그 발견 -> 재고 미등록은 수량 0으로 간주하도록 `OrderController.approve()`에 방어 추가, 테스트로 재현 후 수정 (별도 커밋).
- 참고(이슈로 남김, 이번 Phase 범위 밖): PRD 메뉴 구조상 "재고 등록/초기화" 메뉴가 없어 모든 주문이 항상 재고 0(전량 부족분)에서 시작한다. 재고관리 메뉴 신설 여부는 추후 논의 필요.
