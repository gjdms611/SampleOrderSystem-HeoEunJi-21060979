...
- 화면에 머무는 동안 경과시간 기반으로 진행률이 실시간처럼 계속 올라간다. 화면을 나가면 갱신이 멈춘다.
- 생산이 완료되어 CONFIRMED로 전환된 주문은 그 즉시(화면이 열려있는 동안) 반영된다 (`OrderController.complete_production()` 연동).
...
### 25-D: 조회 화면 전용 스레드 + 상세 출력 (view + main_controller)
- `view/console_view.py`에 위 조합된 정보를 표로 그리는 함수 추가 (라인별 상태 요약 + 현재시각 + 처리중 상세 표 + 대기중 상세 표).
- `controller/main_controller.py`의 "생산라인 조회" 처리:
  1. 별도 스레드를 시작해 0.5초 간격으로 `tick()` 호출 + `describe_current`/`describe_waiting`으로 화면을 다시 그림.
  2. 메인 스레드는 사용자의 Enter 입력을 기다린다.
...
- [ ] (25-A) `job.started_at`이 `assign_idle_lines()` 시점에 설정되는지 테스트
- [ ] (25-B) `tick()`이 경과시간에 비례해 `produce_unit()`을 반복 호출하고, 완료된 job에 대해 `order_controller.complete_production()`까지 연동되어 Order.status가 CONFIRMED로 바뀌는지 테스트 (`time.sleep` 없이 `started_at`을 과거로 세팅해 결정론적으로 검증)
- [ ] (25-C) 처리중/대기중 목록 조합 메서드가 요구된 필드(주문번호/시료정보/주문량/재고/부족분/실생산량/수율/진행률/예상완료시각 등)를 정확히 채우는지 테스트
- [ ] (25-D) `python main.py`로 "생산라인 조회" 진입 시 라인개수/상태/현재시각/처리중 상세/대기중 상세가 보이고, 화면에 머무는 동안 진행률이 실시간으로 올라가며, 나가면 스레드가 정리되는지 수동 확인
