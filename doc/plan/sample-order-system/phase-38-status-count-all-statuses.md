...
- [x] 주문이 하나도 없을 때도 `count_orders_by_status()`가 5개 상태 전부 0으로 반환하는지 테스트
- [x] 주문이 일부 상태에만 있을 때, 없는 상태는 0으로 채워지는지 테스트
- [x] `python main.py`로 모니터링 메뉴에서 주문상태별 카운트가 항상 5개 행을 보여주는지 수동 확인
