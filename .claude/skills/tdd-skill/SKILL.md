...
하나의 PRD에서 나온 작업을 Plan.md 하나에 통짜로 담아 한 번의 RED-GREEN-REVIEW로 밀어붙이면, 테스트/구현 단위가 너무 커져서 "실패하는 이유가 명확한지", "통과시킬 최소 코드가 무엇인지" 판단하기 어려워진다. 그래서 `doc/plan/<기능명>.md`를 작성할 때 작업을 여러 **Phase**로 나눈다 — 각 Phase는 그 자체로 하나의 RED-GREEN-REVIEW 사이클이 될 만큼 작은, 독립적으로 검증 가능한 동작 단위다.

- Plan.md 작성 시점(RED 진입)에 전체 Phase 목록을 먼저 스케치한다 (Phase 1, Phase 2, ... 각각 한 줄 목표 + 체크박스).
- 각 Phase 아래에는 `설계(Design)` 섹션을 두고, 그 Phase를 구현할 상세 설계(접근 방식, 주요 함수/클래스 시그니처, 데이터 구조, 다뤄야 할 엣지 케이스)를 적는다. 이 설계가 이후 그 Phase의 테스트/구현을 쓸 때 근거가 된다 — 구현하면서 즉흥적으로 정하지 말고, 미리 설계해둔 것을 테스트로 검증하고 코드로 옮긴다.
- Action은 Phase 목록을 한 번에 다 구현하는 게 아니라, Phase마다 그 Phase의 설계를 기준으로 별도의 RED-GREEN-REVIEW-커밋을 반복한다. 즉 하나의 Plan.md 안에서 여러 번의 작은 사이클이 Phase 순서대로 돈다.
- 한 Phase의 REVIEW+커밋이 끝나면 Plan.md에서 그 Phase의 체크박스만 완료로 표시하고, 다른 Phase의 내용(설계, 아직 진행 안 한 것 포함)은 건드리지 않는다. 다음 Phase로 넘어가 그 Phase의 설계를 참고해 RED부터 다시 시작한다.
- Phase를 진행하다 범위나 설계가 예상과 달라 바꿔야 한다면, 해당 Phase의 설계 항목만 갱신하고 사람 파트너에게 다시 승인받는다 — 이미 완료 커밋된 다른 Phase는 손대지 않는다.
...
### 1. RED - 목표 설정 + Plan + 실패하는 테스트 + 인간 검토 + 커밋

무엇이 일어나야 하는지를 정확히 설정하라. 모호하면 구현으로 넘어가지 말고 사람 파트너에게 물어라 — 잘못된 목표를 향해 완벽하게 구현해봤자 되돌리는 비용이 더 크다.

**`doc/plan/<기능명>.md` 작성 (최초 1회)**: 이번 기능 전체를 Phase 목록으로 쪼개고, 각 Phase 아래에 설계(Design)를 적는다. 사람이 코드를 보기 전에 "무엇을, 왜, 어떻게"를 Phase 단위로 승인할 수 있는 체크포인트다.

**Phase별 RED**: 지금 진행할 Phase의 설계를 근거로 그 Phase가 요구하는 동작 하나에 대한 실패하는 테스트를 작성하라. 아래는 여전히 그대로 지킨다 — Phase의 설계가 그 위에 사람이 개입할 지점을 추가하는 것이지, 대체하는 게 아니다:
...
**인간 검토 + 커밋**: (Plan.md가 새로 작성된 경우) Phase 목록+설계와, 지금 Phase의 실패하는 테스트를 사람 파트너에게 제시하고 검토를 요청하라. 승인 전에는 구현(GREEN)으로 넘어가지 말라. 승인받으면 `doc/plan/<기능명>.md`(신규/변경분) + 테스트를 커밋하라 — 이 커밋이 "우리가 무엇을 만들기로 합의했는가"의 기록이 된다.
...
기능을 추가하지 말고, 다른 코드를 리팩터링하지 말고, 이번 Phase의 설계와 테스트가 요구하는 것 이상으로 "개선"하지 말라.
...
이 단계에서는 아직 커밋하지 않는다 — REVIEW까지 기다려라. 사람 파트너가 이번 Phase의 설계와 최종 결과물을 한 번에 같이 볼 수 있어야 계획 이탈 여부를 판단하기 쉽다.

### 3. REVIEW - 계획 대비 코드 검토 + 인간 검토 + 커밋

GREEN에서 작성된 코드를 이번 Phase의 설계와 대조해 검토하라:
- 설계에 없는 것이 구현되지는 않았는가? (스코프 밖 추가 여부)
- 이번 Phase의 목표가 실제로 달성됐는가?
- 리팩토링이 필요한가?

리팩토링이 필요하면(중복 제거, 이름 개선, 헬퍼 추출) 수행하고, 테스트가 여전히 그린인지 재확인하라. 리팩토링 중 동작을 추가하지 말라.

**인간 검토 + 커밋**: 최종 코드(와 리팩토링 결과)를 사람 파트너에게 제시하고 검토를 요청하라. 승인되면 구현을 커밋하라. 반려되면 지적된 부분을 GREEN/REVIEW로 돌아가 고치고 다시 검토받아라. 커밋 후 `doc/plan/<기능명>.md`에서 이번 Phase의 체크박스만 완료로 표시한다 — 다른 Phase의 내용은 건드리지 않는다.

### 반복

같은 `doc/plan/<기능명>.md`의 다음 Phase에 대해 RED부터 다시 시작하라. 모든 Phase가 끝나면 이번 기능 사이클이 완료된다.
...
**REVIEW**
이번 Phase의 설계와 대조: 이메일 외 다른 필드 검증이 추가되지 않았는지 확인. 지금은 검증 규칙이 하나뿐이라 리팩토링(로직 추출) 불필요 — 여러 필드에 대한 검증이 필요해지면 그때 추출한다. 사람 파트너에게 최종 코드를 보여주고 승인받은 뒤 커밋하고, Plan.md에서 이 Phase만 완료 체크한다.

## 검증 체크리스트

한 Phase를 완료(complete)로 표시하기 전에:

- [ ] `doc/plan/<기능명>.md`에 Phase 목록과 이번 Phase의 설계가 존재하고 사람 파트너의 승인을 받았다
...
- [ ] 구현이 이번 Phase의 설계 범위를 벗어나지 않았다 (스코프 밖 추가 없음, 다른 Phase 건드리지 않음)
- [ ] 최종 코드가 사람 파트너의 검토와 승인을 받았다
- [ ] RED(계획+테스트)와 REVIEW(구현) 각각 별도로 커밋되었다
- [ ] 완료된 Phase의 체크박스만 갱신했고, 다른 Phase의 설계 내용은 손대지 않았다
